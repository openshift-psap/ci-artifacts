...
validate_sro_deployment() {
    trap "./run.py sro capture_deployment_state" EXIT

    ./run.py sro run_e2e_test "${CI_IMAGE_SRO_COMMIT_CI_REPO}" "${CI_IMAGE_SRO_COMMIT_CI_REF}"
}
...
test_master_branch() {
    ...
    prepare_cluster_for_sro
    ./run.py sro deploy_from_commit "${CI_IMAGE_SRO_COMMIT_CI_REPO}" \
