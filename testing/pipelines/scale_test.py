...
def customize_rhods():
    if not config.ci_artifacts.config.ci_artifacts.get_config("rhods.operator.stop"):
        return
...
def prepare_test_driver_namespace():
    ...
    istag = config.ci_artifacts.get_command_arg("utils build_push_image --prefix base_image", "_istag")

    if run.run(f"oc get istag {istag} -n {namespace} -oname 2>/dev/null", check=False).rc == 0:
    ...
    secret_name = config.ci_artifacts.config.ci_artifacts.get_config("secrets.dir.name")
    secret_env_key = config.ci_artifacts.config.ci_artifacts.get_config("secrets.dir.env_key")
...
def prepare_sutest_scale_up():
    ...
    if config.ci_artifacts.config.ci_artifacts.get_config("clusters.sutest.is_metal"):
        return

    node_count = config.ci_artifacts.config.ci_artifacts.get_config("clusters.sutest.compute.machineset.count")
    extra = ""
...
def pipelines_run_one():
    ...
    if job_index := os.environ.get("JOB_COMPLETION_INDEX"):
        namespace = config.ci_artifacts.config.ci_artifacts.get_config("rhods.pipelines.namespace")
...
def cleanup_scale_test():
    ...
    #
    label_key = config.ci_artifacts.config.ci_artifacts.get_config("rhods.pipelines.namespace_label.key")
    label_value = config.ci_artifacts.config.ci_artifacts.get_config("rhods.pipelines.namespace_label.value")
...
def cleanup_cluster():
    ...
    #
    base_image_ns = config.ci_artifacts.config.ci_artifacts.get_config("base_image.namespace")
...
def test_ci():
    ...
    try:
        ...
        if config.ci_artifacts.config.ci_artifacts.get_config("clusters.cleanup_on_exit"):
