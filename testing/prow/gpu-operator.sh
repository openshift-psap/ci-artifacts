...
collect_must_gather() {
    run_in_sub_shell() {
        ...
        OPERATOR_IMAGE=$(oc get pods -A -lapp=gpu-operator -o=jsonpath='{range .items[*]}{range .spec.containers[*]}{.image}{end}{end}' || true)
...
case ${action} in
    "test_master_branch")
        ...
        ;;
    "test_commit")
        test_commit https://gitlab.com/kpouget_psap/gpu-operator must-gather
        exit 0
        ;;
    "test_operatorhub")
        if [[ -z "$*" ]]; then
